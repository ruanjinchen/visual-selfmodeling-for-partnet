...
def load_gt_points(data_dir: Path, key: str) -> np.ndarray:
    """
    读取 GT 点云：优先 data_dir/mesh_{key}.xyzn（取前三列为 xyz）；
    若不存在则尝试从 GT 网格采样。
    """
...
python infer_test_all_states_ply.py \
  ...
  --split-json assets/datainfo/multiple_models_data_split_dict_1.json \
  --out-dir eval_out_1118/eyeglasses \
  --res 160 \
  --iso 0.0 \
  --device cuda \
  --keep-largest \
...
python infer_test_all_states_ply.py \
  ...
  --split-json assets/datainfo/multiple_models_data_split_dict_2.json \
  --out-dir eval_out_1118/pliers \
  --res 160 \
  --iso 0.0 \
  --device cuda \
  --keep-largest \
...
python infer_test_all_states_ply.py \
  ...
  --data-dir /data/fllm/code/vsm/data/scissors_10893 \
    --split-json assets/datainfo/multiple_models_data_split_dict_3.json \
  --out-dir eval_out_1118/scissors \
  --res 160 \
  --iso 0.0 \
  --device cuda \
  --min-comp-verts 0
...
python infer_test_all_states_ply.py \
  ...
  --split-json assets/datainfo/multiple_models_data_split_dict_4.json \
  --out-dir eval_out_1118/boston_dynamics_spot \
  --res 160 \
  --iso 0.0 \
  --keep-largest \
  --device cuda \
...
python infer_test_all_states_ply.py \
  ...
  --split-json assets/datainfo/multiple_models_data_split_dict_5.json \
  --out-dir eval_out_1118/franka_fr3 \
  --res 160 \
  --iso 0.0 \
  --device cuda \
  --keep-largest \
