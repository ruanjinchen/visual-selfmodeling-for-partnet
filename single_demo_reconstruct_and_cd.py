# -*- coding: utf-8 -*-
"""
single_single_demo_reconstruct_and_cd.py  —— 多DoF + 两种可选剪枝（GT距离/梯度）版

- 支持多关节：--state-key / --state-rad / --state-deg（三选一，仍兼容 --angle-*）
- 不做任何空间缩放（仅对“状态量”做 angle/π 数值归一化以匹配训练）
- Marching Cubes 重建后可选两类剪枝：
  (1) --prune-by-gt <dist>  ：保留“与GT点云距离<=dist”的三角面 这个在剪刀的demo里不要开，要不没东西了就
  (2) --grad-thr   <thr>    ：保留“所有顶点 |∇SDF|>=thr”的三角面（thr≈0.4~0.7 常用）
- 原demo有的东西：AABB设定/自动、keep-largest、min-comp-*, 采样点云、CD计算等

注意：
- 剪枝均在 MC 之后、导出之前进行；不会引入坐标尺度变化，也不影响后续计算的CD/EMD。
这里的cd就看个乐，并不是非常准确的
"""
...
python single_demo_reconstruct_and_cd.py \
  --ckpt pliers_11_12_state-condition_new-global-siren-sdf_1/lightning_logs/version_4/checkpoints/epoch=299-step=4500.ckpt \
...
python single_demo_reconstruct_and_cd.py \
  --ckpt eyeglasses_11_12_state-condition_new-global-siren-sdf_1/lightning_logs/version_2/checkpoints/epoch=299-step=4500.ckpt \
...
[剪刀过]
scissors_11_12_state-condition_new-global-siren-sdf_1/lightning_logs/version_0/checkpoints/epoch=133-step=2010.ckpt
scissors_11_12_state-condition_new-global-siren-sdf_1/lightning_logs/version_2/checkpoints/epoch=73-step=1110.ckpt
scissors_11_12_state-condition_new-global-siren-sdf_1/lightning_logs/version_5/checkpoints/epoch=299-step=4500.ckpt 最新的 cd最低

python single_demo_reconstruct_and_cd.py \
  --ckpt scissors_11_12_state-condition_new-global-siren-sdf_1/lightning_logs/version_5/checkpoints/epoch=299-step=4500.ckpt \
...
FINAL_boston_dynamics_spot_state-condition_new-global-siren-sdf_1/lightning_logs/version_1/checkpoints/epoch=299-step=4500.ckpt 
original_boston_dynamics_spot_state-condition_new-global-siren-sdf_1/lightning_logs/version_0/checkpoints/epoch=138-step=2085.ckpt
python single_demo_reconstruct_and_cd.py \
  --ckpt original_boston_dynamics_spot_state-condition_new-global-siren-sdf_1/lightning_logs/version_0/checkpoints/epoch=138-step=2085.ckpt \
...
机械臂新版数据一直失败
franka_fr3_state-condition_new-global-siren-sdf_1/lightning_logs/version_6/checkpoints/epoch=375-step=5640.ckpt

FINAL_franka_fr3_state-condition_new-global-siren-sdf_1/lightning_logs/version_0/checkpoints/epoch=299-step=4500.ckpt

python single_demo_reconstruct_and_cd.py \
  --ckpt franka_fr3_state-condition_new-global-siren-sdf_1/lightning_logs/version_6/checkpoints/epoch=375-step=5640.ckpt \
...
python single_demo_reconstruct_and_cd.py \
  --ckpt original_franka_fr3_state-condition_new-global-siren-sdf_1/lightning_logs/version_1/checkpoints/epoch=86-step=1305.ckpt \
